...
const leagueOps_service_1 = require("../services/leagueOps.service");
const enums_1 = require("../enums/enums");
// Typed convenience reference to the prisma mock
const p = prisma_client_1.default;
const makeRoster = (overrides = {}) => ({
    recordId: 1,
...
beforeEach(() => {
    ...
    p.$transaction.mockImplementation((promises) => Promise.all(promises));
...
describe('weeklyUpdate', () => {
    it('awards MAXI_CHALLENGE_WIN points to the maxi winner\'s roster', async () => {
        ...
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, ['Sasha'], false, [], [], [], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            where: { recordId: 1 },
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.MAXI_CHALLENGE_WIN },
            }),
        }));
    });
    it('awards SNATCH_GAME_WIN (30) instead of MAXI_CHALLENGE_WIN (25) when isSnatchGame is true', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, ['Katya'], true, [], [], [], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.SNATCH_GAME_WIN },
            }),
        }));
    });
    it('applies ELIMINATED penalty to an eliminated queen\'s roster', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, [], false, [], [], [], [], [], ['Aja']);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.ELIMINATED },
            }),
        }));
    });
    it('adds MINI_CHALLENGE_WIN on top of a SAFE placement (additive)', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, [], false, ['Nina'], [], ['Nina'], [], [], []);
        const expected = enums_1.PointManipulation.SAFE_PLACEMENT + enums_1.PointManipulation.MINI_CHALLENGE_WIN;
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
    ...
    it('maxi winner overwrites top placement (maxi takes priority)', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, ['Trinity'], false, [], ['Trinity'], [], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.MAXI_CHALLENGE_WIN },
            }),
    ...
    it('scores zero for queens not in any placement category', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklyUpdate)('US', 17, ['Sasha'], false, [], ['Trinity'], ['Nina'], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
    ...
    it('returns null when no rosters are found', async () => {
        ...
        const result = await (0, leagueOps_service_1.weeklyUpdate)('US', 17, [], false, [], [], [], [], [], []);
...
describe('weeklySurvey', () => {
    it('awards GOOD_RUNWAY points for a toot', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklySurvey)(['Shea'], [], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.GOOD_RUNWAY },
            }),
        }));
    });
    it('applies BAD_RUNWAY penalty for a boot', async () => {
        ...
        p.roster.findMany.mockResolvedValue([roster]);
        await (0, leagueOps_service_1.weeklySurvey)([], ['Alexis'], [], [], []);
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: enums_1.PointManipulation.BAD_RUNWAY },
            }),
        }));
    });
    it('stacks ICONIC_MOMENT on top of a toot', async () => {
        ...
        await (0, leagueOps_service_1.weeklySurvey)(['Monet'], [], ['Monet'], [], []);
        const expected = enums_1.PointManipulation.GOOD_RUNWAY + enums_1.PointManipulation.ICONIC_MOMENT;
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: expected },
            }),
...
describe('addUserToLeague', () => {
    it('returns null if the user is already in the league', async () => {
        ...
        const result = await (0, leagueOps_service_1.addUserToLeague)('existing@werk.com', 'MyTeam', league, [], 'US', 17);
        expect(result).toBeNull();
    ...
    it('returns null if the league is at max capacity', async () => {
        ...
        const result = await (0, leagueOps_service_1.addUserToLeague)('newuser@werk.com', 'MyTeam', league, [], 'US', 17);
        expect(result).toBeNull();
        expect(p.league.update).not.toHaveBeenCalled();
    });
    it('creates a roster and updates the league when the user can join', async () => {
        ...
        const result = await (0, leagueOps_service_1.addUserToLeague)('newuser@werk.com', 'MyTeam', league, ['QueenA'], 'US', 17);
        expect(p.league.update).toHaveBeenCalledTimes(1);
        expect(p.roster.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
...
describe('removeUserFromLeague', () => {
    it('does not call prisma.update if the user is not in the league', async () => {
        ...
        await (0, leagueOps_service_1.removeUserFromLeague)('nothere@werk.com', league);
        expect(p.league.update).not.toHaveBeenCalled();
    });
    it('updates the league users array when removing a valid member', async () => {
        ...
        await (0, leagueOps_service_1.removeUserFromLeague)('remove@werk.com', league);
        expect(p.league.update).toHaveBeenCalledWith(expect.objectContaining({
            where: { id: league.id },
...
describe('computeFanSurvey', () => {
    it('returns null when there are no survey responses', async () => {
        ...
        const result = await (0, leagueOps_service_1.computeFanSurvey)('US', 17, 1);
        expect(result).toBeNull();
    });
    it('awards the correct fan survey points to the plurality queen', async () => {
        ...
        await (0, leagueOps_service_1.computeFanSurvey)('US', 17, 1);
        const expected = enums_1.FanSurveyPoints.QUEEN_OF_WEEK + enums_1.FanSurveyPoints.BEST_DRESSED;
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: expected },
            }),
    ...
    it('awards fan survey points to ALL queens tied for the plurality', async () => {
        ...
        const sashaCall = calls.find((c) => c[0].where.recordId === 1);
        const katyaCall = calls.find((c) => c[0].where.recordId === 2);
        // Both tied queens should receive QUEEN_OF_WEEK points
        expect(sashaCall[0].data.currentPoints.increment).toBe(enums_1.FanSurveyPoints.QUEEN_OF_WEEK);
        expect(katyaCall[0].data.currentPoints.increment).toBe(enums_1.FanSurveyPoints.QUEEN_OF_WEEK);
    });
    it('applies negative points for bottom-of-week and worst-dressed winners', async () => {
        ...
        await (0, leagueOps_service_1.computeFanSurvey)('US', 17, 1);
        const expected = enums_1.FanSurveyPoints.BOTTOM_OF_WEEK + enums_1.FanSurveyPoints.WORST_DRESSED;
        expect(p.roster.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                currentPoints: { increment: expected },
            }),
